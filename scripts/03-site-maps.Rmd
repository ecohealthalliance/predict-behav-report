...
# Read in data
dat0 <- read_csv(h(paste0("data/site-maps-full-", country, ".csv")))
dat1 <- read_csv(h(paste0("data/site-maps-agg-", country, ".csv")))
...
age_pyramid <- function(dat, site) {
    filt <- filter(dat1 == dat, site == site)
...
plots <- lapply(seq_along(unique(pyramids$site)), function(x) {
    name <- unique(pyramids$site)[x]
    png(filename = paste0(h("outputs/age-plots/"), name, ": age-dist.png"), width = 500, height = 300) 
    age_pyramid(pyramids, unique(pyramids$site)[x])
    dev.off()
})
...
#psub$pngpath
psub <- pyramids %>% 
    select(country, site, new, old, lat = concurrent_lat, long = concurrent_long, size) %>% 
    group_by(site) %>% 
    slice(1) %>% 
    mutate(pngpath = paste0(h("outputs/age-plots/"), site, ": age-dist.png"))
...
hunt <- dat0 %>% 
    ...
    summarize(country = first(country), 
              ...
              n = n()) %>% 
    mutate(lifelab = paste0(site, ": ",percent(life_prop), " (", life, "/", n, " respondents)"),
           yearlab = paste0(site, ": ",percent(year_prop), " (", year, "/", n, " respondents)"))
...
contact_prev <- function(site, dat = site_props) {
    ...
    # Generate the ggplot 
    name = paste0(h("outputs/animal-contact-figs/"), y$site[1], "-animal-contact.png")
...
psub2 <- dat1 %>% 
    mutate(pngpath = paste0(h("outputs/animal-contact-figs"), site, ".png"))

leaflet(psub2) %>%
    setView(lng = mean(dat1$long), lat = mean(dat1$lat)-10, zoom = 3) %>% 
    addProviderTiles("OpenStreetMap.Mapnik") %>% 
    addCircleMarkers( 
        data = psub, lng = ~long, lat = ~lat, color = "black", opacity = 1, weight = 1,
        popup = c(
...
# taxa 
taxa <- dat0 %>% 
    select(participant_id, country, site, n, lat, long, bats_contact, nhp_contact, rodents_contact, poultry_contact, swine_contact)

# subset
# dat1 <- dat1 %>%
#   select(site, country, lat, long, new, n, hovertext)
...
all_by_taxa <- map_df(taxa_cols, function(x) {
    map_df(unique(taxa$site), function(z) {
...
map(animals, function(x) {
    map(unique(taxa$site), function(z) {
        ggplot_taxa(z, x)
    })
})
...
coords <- coords %>% 
  mutate(npath = popupImage3(paste0(h("outputs/taxa-site-maps/non-human primates/"), site, ".png")), 
         bpath = popupImage3(paste0(h("outputs/taxa-site-maps/bats/"), site, ".png")), 
         rpath = popupImage3(paste0(h("outputs/taxa-site-maps/rodents/"), site, ".png")))
