...
```{r map-questionnaires, eval=TRUE}
# Set color palette 
...
# Map 
leaflet(adat) %>%
  setView(lng = mean(adat$site_longitude), lat = mean(adat$site_latitude)-10, zoom = 3) %>%
...
```{r table-questionnaires, eval=TRUE}
adat %>%
  arrange(-n) %>%
...
iwalk(age_split, function(x, y){
  ...
  ggplot(x, aes(x = agebrack, y = ifelse(gender == "male",-1, 1), fill = gender)) + 
    geom_col() + 
    theme_avenir(base_family = "Avenir") + 
    coord_flip() +
    labs(x = "age group", y = "count", main = title) + 
...
leaflet(age_map) %>%
  ...
  addCircleMarkers( 
    ...
    popup = c(popupImage3(age_map$age_pngpath[1]),
              popupImage3(age_map$age_pngpath[2]),
              popupImage3(age_map$age_pngpath[3]),
              popupImage3(age_map$age_pngpath[4]),
              popupImage3(age_map$age_pngpath[5]) )) %>%
  addPolygons(
    data = admin, opacity = 100,
    color = "#000000",
    weight = 1, popup = NULL,
...
hunt <- fdat %>% 
  ...
  summarize(life = sum(hunted_animal_life), 
...
# map hunting
leaflet(hunt) %>%
  setView(lng = mean(adat$site_longitude), lat = mean(adat$site_latitude)-10, zoom = 4) %>%
  addProviderTiles("OpenStreetMap.Mapnik") %>%  
  addCircleMarkers(
  ...
  addCircleMarkers(
    ...
    label = ~lifelab) %>%
  addCircleMarkers(
    lng = ~site_latitude, lat = ~site_latitude, color = "#000000", weight = 1, opacity = 100, 
    radius = ~log(year)*3, 
...
cnames <- c("ID", names(adat), names(fdat[vec]))
...
contact <- fdat %>% 
  ...
  group_by(country, full_site_name, site_name, n, variable, concurrent_sampling_site, site_latitude, site_longitude) %>% 
...
iwalk(contact_split, function(x, y){
  ...
  sub <- paste0("Based on questionnaire responses from ", unique(x$n), " individuals")
  path <- unique(x$contact_pngpath)
  
  
  ggplot(x, aes(x = reorder(question, last_year_prop), y = last_year_prop, 
...
```{r map-contact}
contact_map <- contact %>%
  select(country, full_site_name, site_latitude, site_longitude, n, contact_pngpath) %>% 
  distinct()
...
leaflet(contact_map) %>%
  ...
  addCircleMarkers( 
    ...
    popup = c(
      popupImage3(contact_map$contact_pngpath[1]),
      popupImage3(contact_map$contact_pngpath[2]),
      popupImage3(contact_map$contact_pngpath[3]),
      popupImage3(contact_map$contact_pngpath[4]),
      popupImage3(contact_map$contact_pngpath[5]) )) %>%
  addPolygons(
    data = admin, opacity = 100,
    color = "#000000",
    weight = 1, popup = NULL,
...
taxa_map <- taxa %>%
  ...
  distinct() %>%
  mutate(taxa_pngpath = map(taxa_pngpath, ~popupImage3(.))) %>%
