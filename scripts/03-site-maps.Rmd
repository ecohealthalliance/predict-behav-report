...
fdat <- dat %>% 
  ...
  mutate(lat = mean(site_latitude), long = mean(site_longitude)) %>% 
  ungroup() %>%
  rename(hovertext = new)

adat <- fdat %>%
  select(country, lat, long, concurrent_sampling_site, n, hovertext) %>% 
  unique()
...
data(wrld_simpl)
admin <- wrld_simpl[wrld_simpl@data$NAME %in% country, ]
...
t <- adat %>% 
    group_by(concurrent_sampling_site) %>% 
    summarise(n = sum(n)) %>%
    rename(site = concurrent_sampling_site) %>%
    arrange(-n)
