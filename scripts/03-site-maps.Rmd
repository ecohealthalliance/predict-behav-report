...
fdat <- dat %>% 
  ...
  drop_na(n) # this is removing Independent Site which does not have a lookup in site_names...
...
contact <- fdat %>%
  ...
  dplyr::select(cnames, gender) %>%
  gather(-c(ID:n, gender), key = "variable", value = "value") %>%
  ...
  group_by(country, gender, variable) %>%
  summarise(n = n(), 
            last_year = sum(last_year, na.rm = TRUE),
            last_year_prop = sum(last_year, na.rm = TRUE)/n()) %>%
  ungroup() %>%
  left_join(., contact_names, by = "variable") %>%
  mutate(contact_pngpath = paste0(h("outputs/animal-contact-figs/"), country,  "-animal-contact.png")) %>%
  mutate(last_year_prop = ifelse(gender == "female", last_year_prop, last_year_prop * -1))
...
y <- "Bangladesh"
  ...
  cap <- paste0("\nBased on questionnaire responses from ", unique(x$n[x$gender == "male"]), " male and ", unique(x$n[x$gender == "female"]), " female individuals")
  path <- unique(x$contact_pngpath)
  
  p <- ggplot(x, aes(x = reorder(question, abs(last_year_prop)), y = last_year_prop,
                     fill = gender)) +
    geom_col() +
    #geom_text(aes(label = scales::percent(last_year_prop)), size = 15, hjust = -0.25) +
    theme_fivethirtyeight() +
    scale_y_continuous(breaks = c(-1, -0.5, 0, 0.5, 1.0), labels = c(1, 0.5, 0, 0.5, 1.0)) +
    coord_flip() +
    labs(x = NULL, 
    ...
    theme(text = element_text(size = 50), 
          ...
          plot.subtitle = element_text(hjust=-3, size = 50),
