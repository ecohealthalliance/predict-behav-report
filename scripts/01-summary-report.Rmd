...
params:
  country: ""
title: '<span style="color:red;">DRAFT</span> Summary Behavioral Report: `r params$country`'
date: "`r Sys.Date()`"
output:
  html_document:
    keep_md: false
...
community_engagement_sites <- get_sum_table(dat %>% drop_na(community_engagement), 
                                            pretty_names = site_names, 
...
liv_table_all <- get_sum_table(dat, 
                               ...
                               factor_levels = c("extraction of minerals, gas, oil, timber",
                                                 "crop production",
...
age_total <-  suppressWarnings( dat %>%
                                  ...
                                  gather() %>%
                                  mutate(Aggregate = round(value, 0)) %>%
                                  select(-value) 
)
...
age_table <- dat %>%
  ...
  left_join(age_total, by = c("rowname"="key")) %>%
...
occ_total <- dat %>%
  summarize(`Number of Children in Household (average)` = mean(children_in_dwelling, na.rm = TRUE),
            ...
            `Crowding Index (people per room; average)` = mean(crowding_index, na.rm = TRUE)) %>%
  gather() %>%
  mutate(Aggregate = ifelse(is.nan(value), "--", round(value, 1))) %>%
  select(-value)
...
occ_table <- dat %>%
  ...
  header_true() %>%
  as_tibble() %>%
  mutate_if(is.factor, function(x) round(as.numeric(as.character(x)), 1) %>% replace_na("--")) %>%
  left_join(occ_total, by = c("rowname"="key")) %>%
  mutate_if(is.numeric, as.character) %>%
  rename_at(site_names$old, function(x) site_names$new[site_names$old == x]) %>%
  rename(` ` = rowname) %>%
  select(` `, site_names$new) 
...
dat$contact_all[is.na(dat$contact_all)] = "N/A"
contact_table <- get_sum_table(dat, 
                               pretty_names = site_names, 
                               group_var = "contact_all", 
                               factor_levels = c("bats", 
                                                 "birds",
                                                 "camels",
                                                 "carnivores",
                                                 "cats",
                               ...
                               factor_lab = c("bats",
                                              ...
                                              "ungulates",
                                              "no taxa contact"),
                               table_lab = "Taxa Contact (past year)", 
                               asterisk = TRUE)

# Together 
out <- bind_rows(act_table, act_table2, risk_table, risk_table2, worried_table, contact_table)
