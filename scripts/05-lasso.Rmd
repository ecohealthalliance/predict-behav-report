...
for (endpt in c(illness_outcomes, taxa_outcomes)){
  ...
  model_dat <- read_csv(h("lasso", paste(country, endpt, "lasso-dat.csv", sep = "-")))
  lasso_fit <- read_rds(h("lasso", paste(country, endpt, "lasso-fit.rds", sep = "-")))
  
  if(!is.infinite(lasso_fit$lambda)){
    p <- plot_lasso_fit(lasso_fit, model_dat, pretty_names = TRUE, vert_line = FALSE) 
    
    ggsave(plot = p, file = h("outputs/lasso-figs", paste(country, endpt, "lasso.png", sep = "-")), width = 10, height = 7, units = "in")
  }else{
...
```{r plots,  results='asis', fig.align='center', fig.width=10, fig.height=7}
