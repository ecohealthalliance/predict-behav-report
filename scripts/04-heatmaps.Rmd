...
# Get site names
site_names <- read_csv(h("site-name-lookup.csv"), col_types = cols()) %>%
...
age_dat <- bind_cols(dat %>% select(age_group), ldat)
...
hm <- get_heatmap(site_dat, taxa_types = taxa_outcomes, group_var = "new", title = "Taxa Contact by Site")
