...
write_csv(ldat, h(paste0("data/logic-behav-", country_mod, ".csv"))) # used in heat maps and lasso

tdat <- dat %>%
  get_tab(.)
write_csv(tdat, h(paste0("data/tab-behav-", country_mod, ".csv"))) # used in tabular report

# Load and process site names
site_lookup <- read_csv(h("site-name-lookup.csv"), col_types = cols()) %>% 
  get_site_names(dat, ., country)
write_csv(site_lookup, h(paste0("data/site-name-formatted-", country_mod, ".csv")))
