...
########## Create functions to use for report ##########

get_all_dat <- function(dat, adat, country){ # input dataframe in format of site-name-lookup.csv
  
adat %>%
    filter(country == !!country, 
           old %in% unique(dat$concurrent_sampling_site)) %>%
    right_join(dat, by = c("old" = "concurrent_sampling_site")) %>%
    group_by(new) %>%
    rename(site = new) %>%
...
get_map_dat <- function(dat){
  
  mdat <- dat %>%
    group_by(site) %>% 
    select(country, lat, long, site, n, hovertext) %>% 
    unique()
}